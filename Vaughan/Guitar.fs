...
module Guitar =
    ...
    module private GuitarFrets =
        ...
        let raiseOctave (fret: Fret) =
            { fret with
                  Fret = fret.Fret + int (toDistance PerfectOctave) }
        ...
        let unstretch frets =
            frets
            |> List.map (fun f -> unstretchFret (highestFret frets) f)
    ...
    module private GuitarStrings =
        ...
        let createStringFret guitarString note =
            { createMutedStringFret guitarString with
                  Fret = int (fretNoteOnString note guitarString)
                  Note = note }
    ...
    module private MapDropChords =
        ...
        let private skipString bassString chord guitarString =
            chord.ChordType = Drop3
            && guitarString = nextString bassString
        ...
        let private mapChordToGuitarFrets bassString chord =
            ...
            let rec mapChordNoteToString guitarString chordNotes mappedChordNotes =
                ...
                | _ ->
                    let fret =
                        mapNoteToFret guitarString (fst chordNotes.[0]) shouldSkipString

                    let unmapedChordNotes =
                        unmapedChordNotes chordNotes shouldSkipString guitarString
        ...
        let dropChordToGuitarChord bassString chord =
            ...
            let closedChord =
                { guitarChord with
                      Frets = raiseOpenFrets guitarChord.Frets }

            { closedChord with
                  Frets = unstretch closedChord.Frets }
    ...
    module private MapNonDropChords =
        let private mapAllChordNotesToFretsOnString allowedFrets guitarStringIndex (chord: Chord) =
            ...
            [ for chordNoteIndex in 0 .. (chord.Notes.Length - 1) do
                yield (createStringFret guitarString (fst chord.Notes.[chordNoteIndex])) ]
        ...
        let private generateAllFretCombinations allowedFrets bassString chord =
            ...
            [ for guitarStringIndex in 1 .. bassStringOrdinal do
                yield (mapAllChordNotesToFretsOnString allowedFrets guitarStringIndex chord) ]
            |> combineAll
    ...
    module private MapMelodicLines =
        ...
        let private generateAllFretCombinations filterFrets (notes: Note list) =
            [ for guitarStringIndex in 1 .. 6 do
                yield (mapAllNotesToFretsOnString filterFrets guitarStringIndex notes) ]
            |> List.collect id

        let private filterAllowedFrets allowedFrets (frets: Fret list) =
            frets
            |> List.map (fun f -> if allowedFrets f then f else raiseOctave f)
            |> List.filter allowedFrets

        let chordToGuitarArpeggio allowedFrets chord =
            { BaseChord = chord
              ArpeggioFrets =
                  generateAllFretCombinations (filterAllowedFrets allowedFrets) (chord.Notes |> List.map fst) }
    ...
    let guitarChord: CreateGuitarChord =
        fun bassString chord ->
            ...
            | Closed when chord.Notes
                          |> List.exists (fun n -> snd n = Ninth) -> dropChordToGuitarChord bassString chord
...
module GuitarTab =
    ...
    let private endTab =
        ("-||" + Environment.NewLine) |> List.replicate 6
    ...
    module private Tabify =
        ...
        let private renderChordNote (mutedStringTab: string) (fret: Fret) highestFet =
            ...
            else if highestFet > 9 then if fret.Fret > 9 then sprintf "-%i-" fret.Fret else sprintf "-%i--" fret.Fret
            else sprintf "-%i-" fret.Fret

        let private renderChordNotes mutedStringTab frets =
            let highestFet =
                frets |> List.map (fun f -> f.Fret) |> List.max
        ...
        let private renderNote (noteRenderer: int -> string) (ornament: string) (fret: Fret) =
            let mutedStringTab =
                sprintf "-%s-" (if fret.Fret > 9 then "--" else "-")
        ...
        let private renderChord (chord: GuitarChord) =
            ...
            let mutedStringTab =
                sprintf "-%s-" (if highestFet > 9 then "--" else "-")
        ...
        let private mapTabColumsToTabLines stringOrdinal (tabifiedChords: TabColumns) =
            tabifiedChords
            |> List.map (fun f -> f.[stringOrdinal])

        let mapTabToGuitarStrings (tabifiedChords: TabColumns) =
            [ 0 .. 5 ]
            |> List.map
                ((fun stringOrdinal -> mapTabColumsToTabLines stringOrdinal tabifiedChords)
        ...
        let private renderNotesPredefinedOrder (noteRenderer: int -> string) (ornament: string) (frets: Fret list) =
            frets
            |> List.mapi (fun i n ->
                if i < frets.Length - 1 then renderNote noteRenderer "" n else renderNote noteRenderer ornament n)
    ...
    module private Shapify =
        ...
        let private shapifyFret fret =
            if fret.Fret = -1 then "X" else sprintf "%i" fret.Fret
    ...
    let tabifyAll: TabifyAll =
        fun guitarChords ->
            ...
            @ (guitarChords |> List.map (fun c -> Chord(c)))
            @ [ End ]
            |> renderTab

    let tabifyChord: TabifyChord =
        fun guitarChord -> tabifyAll [ guitarChord ]
    ...
    let shapify: Shapify =
        fun guitarChord ->
            ...
            + "EADGBE"
            + Environment.NewLine
            + (guitarChord |> shapifyChord |> List.fold (+) "")
            + Environment.NewLine

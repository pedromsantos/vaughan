...
module Guitar =
    ...
    module private GuitarFrets =
        ...
        let private isOpen fret = fret.Fret = 0
        let private isMuted fret = fret.Fret = -1
        let private isRaised fret = fret.Fret > 11
        let private fretDistance fret other = abs (fret.Fret - other.Fret)
        let private isStretched fret other = (fretDistance fret other) > 5
        let raiseOctave (fret : Fret) =
            { fret with Fret = fret.Fret + int (toDistance PerfectOctave) }
        let private isRaisable fret = not (isRaised fret) && not (isMuted fret)

        let private unstretchFret highestFret fret =
            if isStretched fret highestFret && isRaisable fret then
                raiseOctave fret
            else fret

        let private highestFret frets = frets |> List.maxBy (fun f -> f.Fret)

        let raiseOpenFrets frets =
            frets
            |> List.map (fun fret ->
                   if isOpen fret then raiseOctave fret
                   else fret)
    ...
    module private GuitarStrings =
        ...
        type private GuitarStringAttributes =
            { Name : string
              OpenStringNote : Note
              Index : int }

        let private guitarStringAttributes =
            function
            | SixthString ->
                { Name = "Sixth"
                  OpenStringNote = E
        ...
        let createMutedStringFret guitarString =
            { GuitarString = guitarString
              Fret = fretForMutedString
              Note = openStringNote guitarString }

        let createStringFret guitarString note =
            { createMutedStringFret guitarString with Fret =
                                                          int
                                                              (fretNoteOnString
                                                                   note
                                                                   guitarString)
    ...
    module private MapDropChords =
        ...
        let private mapChordToGuitarFrets bassString chord =
            ...
            let rec mapChordNoteToString guitarString chordNotes
                    mappedChordNotes =
                match chordNotes with
                | [] -> mappedChordNotes
                | _ ->
                    let fret =
                        mapNoteToFret guitarString (fst chordNotes.[0])
                            shouldSkipString
                    let unmapedChordNotes =
                        unmapedChordNotes chordNotes shouldSkipString
                            guitarString
                    mapChordNoteToString (nextString guitarString)
                        unmapedChordNotes (fret :: mappedChordNotes)
        ...
        let dropChordToGuitarChord bassString chord =
            ...
            let closedChord =
                { guitarChord with Frets = raiseOpenFrets guitarChord.Frets }
            { closedChord with Frets = unstretch closedChord.Frets }

    [<AutoOpen>]
    module private MapNonDropChords =
        let private mapAllChordNotesToFretsOnString allowedFrets
            guitarStringIndex (chord : Chord) =
            let guitarString = indexToGuitarString guitarStringIndex
            [ for chordNoteIndex in 0..(chord.Notes.Length - 1) do
                  yield (createStringFret guitarString
                             (fst chord.Notes.[chordNoteIndex])) ]
        ...
        let private generateAllFretCombinations allowedFrets bassString chord =
            ...
            [ for guitarStringIndex in 1..bassStringOrdinal do
                  yield (mapAllChordNotesToFretsOnString allowedFrets
                             guitarStringIndex chord) ]
            |> combineAll
    ...
    module private MapMelodicLines =
        let private mapAllNotesToFretsOnString filterFrets guitarStringIndex
            (notes : Note list) =
            let guitarString = indexToGuitarString guitarStringIndex

            let frets =
                [ for noteIndex in 0..(notes.Length - 1) do
                      yield (createStringFret guitarString notes.[noteIndex]) ]
                |> filterFrets
            match frets with
        ...
        let private generateAllFretCombinations filterFrets (notes : Note list) =
            [ for guitarStringIndex in 1..6 do
                  yield (mapAllNotesToFretsOnString filterFrets
                             guitarStringIndex notes) ]
            |> List.collect id

        let private filterAllowedFrets allowedFrets (frets : Fret list) =
            frets
            |> List.map (fun f ->
                   if allowedFrets f then f
                   else raiseOctave f)
            |> List.filter allowedFrets
        ...
        let chordToGuitarArpeggio allowedFrets chord =
            { BaseChord = chord
              ArpeggioFrets =
                  generateAllFretCombinations (filterAllowedFrets allowedFrets)
                      (chord.Notes |> List.map fst) }
    ...
    let guitarChord : CreateGuitarChord =
        fun bassString chord ->
            match chord.ChordType with
            | Drop2
            | Drop3
            | Triad -> dropChordToGuitarChord bassString chord
            | Open -> chordToGuitarOpenChord bassString chord
            | Closed when chord.Notes |> List.exists (fun n -> snd n = Ninth) ->
                dropChordToGuitarChord bassString chord
            | Closed -> chordToGuitarClosedChord bassString chord

    open Chords
    open Scales

    let guitarArpeggio : CreateGuitarArpeggio =
        fun minFret maxFret chord ->
            chord
            |> toClosed
            |> chordToGuitarArpeggio
                   (fun f -> f.Fret >= minFret && f.Fret <= maxFret)

    let guitarScale : CreateGuitarScale =
        fun minFret maxFret scale ->
            scale
            |> scaleToGuitarScale
                   (fun f -> f.Fret >= minFret && f.Fret <= maxFret)
    let guitarNote : CreateGuitarNote = createStringFret
    let guitarNotes : CreateGuitarNotes =
        fun minFret maxFret notes ->
            notes
            |> melodicLineToGuitarMelodicLine
                   (fun f -> f.Fret >= minFret && f.Fret <= maxFret)
    let guitarScaleName (scale : GuitarScale) = scaleName scale.Scale
    let guitarScaleNoteNames (scale : GuitarScale) = scaleNoteNames scale.Scale

    let raiseOctaveVerticaly fretedNote minFret maxFret =
        let octaveString =
            fretedNote.GuitarString
            |> nextString
            |> nextString

        let raised = createStringFret octaveString fretedNote.Note
        if raised.Fret >= minFret && raised.Fret <= maxFret then raised
        else
            let octaveString =
                fretedNote.GuitarString
                |> nextString
...
module GuitarTab =
    ...
    module private Tabify =
        let private stringForLead frets = (frets |> List.last).GuitarString
        let private stringForBass frets = (frets |> List.head).GuitarString
        ...
        let private renderMutedHigherStrings (mutedStringTab : string)
            (frets : Fret list) =
            let mutedStrings = numberOfMutedHighStrings frets
            List.replicate mutedStrings mutedStringTab

        let private renderMutedLowerStrings (mutedStringTab : string)
            (frets : Fret list) =
        ...
        let private renderChordNote (mutedStringTab : string) (fret : Fret)
            highestFet =
            if fret.Fret = -1 then mutedStringTab
            else if highestFet > 9 then
                if fret.Fret > 9 then sprintf "-%i-" fret.Fret
                else sprintf "-%i--" fret.Fret
            else sprintf "-%i-" fret.Fret
        ...
        let private renderChordNotes mutedStringTab frets =
            ...
            frets
            |> List.map
                   (fun fret -> renderChordNote mutedStringTab fret highestFet)
            |> List.rev

        let private renderNote (noteRenderer : int -> string)
            (ornament : string) (fret : Fret) =
            let mutedStringTab =
                sprintf "-%s-" (if fret.Fret > 9 then "--"
                                else "-")
        ...
        let private renderChord (chord : GuitarChord) =
            ...
            let mutedStringTab =
                sprintf "-%s-" (if highestFet > 9 then "--"
                                else "-")
        ...
        let private mapTabColumsToTabLines stringOrdinal
            (tabifiedChords : TabColumns) =
            tabifiedChords |> List.map (fun f -> f.[stringOrdinal])
        let mapTabToGuitarStrings (tabifiedChords : TabColumns) =
            [ 0..5 ]
            |> List.map
                   ((fun stringOrdinal ->
                    mapTabColumsToTabLines stringOrdinal tabifiedChords)
                    >> (fun gss -> gss |> List.fold (fun acc gs -> gs + acc) ""))

        let private renderNotesPredefinedOrder (noteRenderer : int -> string)
            (ornament : string) (frets : Fret list) =
            frets
            |> List.mapi (fun i n ->
                   if i < frets.Length - 1 then renderNote noteRenderer "" n
                   else renderNote noteRenderer ornament n)
            |> mapTabToGuitarStrings

        let private renderNotes (noteRenderer : int -> string)
            (ornament : string) (frets : Fret list) =
            frets
        ...
        let renderTabPart =
            ...
            | ArbArpeggio a ->
                renderNotesPredefinedOrder (sprintf "-%i-") "" a.ArpeggioFrets
            ...
            | PullOff(fs, fe) ->
                renderNotesPredefinedOrder (sprintf "-%i-") "p" [ fe; fs ]
    ...
    module private Shapify =
        ...
        let private shapifyFret fret =
            if fret.Fret = -1 then "X"
            else sprintf "%i" fret.Fret

        let private shapifyFrets frets = frets |> List.map shapifyFret
    ...
    let tabifyAll : TabifyAll =
        fun guitarChords ->
            [ StandardTunning; Start ]
            @ (guitarChords |> List.map (fun c -> Chord(c))) @ [ End ]
            |> renderTab
    let tabifyChord : TabifyChord = fun guitarChord -> tabifyAll [ guitarChord ]

    let shapify : Shapify =
        fun guitarChord ->
            guitarChord.Chord.Name + Environment.NewLine + "EADGBE"
            + Environment.NewLine + (guitarChord
                                     |> shapifyChord
                                     |> List.fold (+) "")
            + Environment.NewLine

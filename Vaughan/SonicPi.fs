...
module SonicPi =
    ...
    let private synthToSonicPySynth =
        function
        | Synths.Beep -> ":beep"
    ...
    let private drumSampleToSonicPi =
        function
    ...
    let private fxToSonicPiFx =
        function
        | BandPassFilter -> "bpf:"
        | BandEQFilter -> ":band_eq"
        ...
        | HighPassFilter -> ":hpf"
        | Krush -> ":krush"
        | LevelAmplifier -> ":level"
        | LowPassFilter -> ":lpf"
        | Mono -> ":mono"
        | NormalisedResonantLowPassFilter -> ":nlpf"
        | NormalisedResonantHighPassFilter -> ":nrbpf"
        | NormalisedHighPassFilter -> ":nhpf"
        | NormalisedLowPassFilter -> ":nrlpf"
        | Normaliser -> ":normaliser"
        | NormalisedBandPassFilter -> ":nbpf"
        | NormalisedResonantBandPassFilter -> ":nrhpf"
        | Octaver -> ":octaver"
        | PanSlicer -> ":panslicer"
        | Pan -> ":pan"
        | PitchShift -> ":pitch_shift"
        | Reverb -> ":reverb"
        | ResonantLowPassFilter -> ":rlpf"
        | ResonantHighPassFilter -> ":rhpf"
        | ResonantBandPassFilter -> ":rbpf"
    ...
    let private playOptionToSonicPiPlayOption =
        function
        | Amplitude(v) -> "amp"
        | Panning(v) -> "pan"
        | Release(v) -> "release"
    ...
    let private fxOptionToSonicPiFxOption =
        function
        | Amp(v) -> "amp"
        | PreAmp(v) -> "pre_amp"
        | Mix(v) -> "mix"
    ...
    let private chordToSonicPi (chord : Chord) octave =
        (Chords.notesMidiNumbers chord octave |> List.fold (fun acc n -> sprintf "%s%i," acc n) "")
            .TrimEnd(',')
    let private scaleToSonicPi (notes : ScaleNotes) octave =
        (Notes.notesMidiNumbers notes octave |> List.fold (fun acc n -> sprintf "%s%i," acc n) "")
            .TrimEnd(',')
    ...
    let private generatePlayOptionsStatments playOptions =
        (playOptions
         |> List.fold
                (fun acc option ->
                sprintf "%s,%s:%.2f" acc (playOptionToSonicPiPlayOption option)
                    (playOptionValue option)) "")
    let private generateFxOptionsStatments fxOptions =
        (fxOptions
         |> List.fold
                (fun acc option ->
                sprintf "%s,%s:%.2f" acc (fxOptionToSonicPiFxOption option)
                    (fxOptionValue option)) "")
    let private generateSonicPiList list =
        (list |> List.fold (fun acc item -> sprintf "%s%.2f," acc (float (item))) "")
            .TrimEnd(',')
    ...
    let private playBeatNotes (beatNotes : BeatNotes) =
        sprintf "play [%s]\n" ((beatNotes |> List.fold (fun acc n -> sprintf "%s%i," acc (midiBeatNote n)) "")
                .TrimEnd(','))
    ...
    let private playBar (bar : Bar) =
        sprintf "%s"
            (bar
             |> List.fold (fun acc b -> sprintf "%s%s" acc (playBarBeat b)) "")
    let private playBars (bars : Bar list) =
        sprintf "%s"
            (bars |> List.fold (fun acc b -> sprintf "%s%s" acc (playBar b)) "")
    ...
    let rec toSonicPiScript =
        ...
        | WithBpm(bpm, sts) ->
            sprintf "with_bpm %i do\n%send" (int (bpm))
                (generateInnerStatments sts toSonicPiScript)
        | PlayNote(note, octave, opts) ->
            sprintf "play %i%s" (midiNumber note octave)
                (generatePlayOptionsStatments opts)
        | PlayChord(chord, octave, opts) ->
            sprintf "play [%s]%s" (chordToSonicPi chord octave)
                (generatePlayOptionsStatments opts)
        | PlaySample(sample, opts) ->
            sprintf "sample %s%s" (sampleToSonicPi sample)
                (generatePlayOptionsStatments opts)
        | Section(ts, key, bars) -> playBars bars
        | PlayArpeggio(notes, octave, times, opts) ->
            sprintf "play_pattern_timed [%s],[%s]%s"
                (scaleToSonicPi notes octave) (generateSonicPiList times)
                (generatePlayOptionsStatments opts)
        | WithFx(fx, opts, sts) ->
            sprintf "with_fx %s%s do\n%send" (fxToSonicPiFx fx)
                (generateFxOptionsStatments opts)
                (generateInnerStatments sts toSonicPiScript)
        | WithSynth(synth, sts) ->
            sprintf "with_synth %s do\n%send" (synthToSonicPySynth synth)
                (generateInnerStatments sts toSonicPiScript)
        | Repeat(repeats, sts) ->
            sprintf "%i.times do\n%send" repeats
                (generateInnerStatments sts toSonicPiScript)
        | LiveLoop(name, sts) ->
            sprintf "live_loop :%s do\n%send" name
                (generateInnerStatments sts toSonicPiScript)

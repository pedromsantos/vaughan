...
module SonicPi =
    ...
    let private generateInnerStatments statments generator =
        (statments
         |> Seq.fold (fun acc st -> sprintf "%s%s\n" acc (generator st)) "")
    ...
    let private generateSonicPiList list =
        (list
         |> List.fold (fun acc item -> sprintf "%s%.2f," acc (float (item))) "").TrimEnd(',')

    let private midiBeatNote ((note, octave, duration): BeatNote) = midiNumber note octave

    let private playBeatNotes (beatNotes: BeatNotes) =
        sprintf "play [%s]\n"
            ((beatNotes
              |> List.fold (fun acc n -> sprintf "%s%i," acc (midiBeatNote n)) "").TrimEnd(','))
    ...
    let private playBars (bars: Bar list) =
        sprintf "%s"
            (bars
             |> List.fold (fun acc b -> sprintf "%s%s" acc (playBar b)) "")
    ...
    let rec toSonicPiScript =
        ...
        | PlayNote (note, octave, opts) ->
            sprintf "play %i%s" (midiNumber note octave) (generatePlayOptionsStatments opts)
        | PlayChord (chord, octave, opts) ->
            sprintf "play [%s]%s" (chordToSonicPi chord octave) (generatePlayOptionsStatments opts)
        | PlaySample (sample, opts) ->
            sprintf "sample %s%s" (sampleToSonicPi sample) (generatePlayOptionsStatments opts)
        | Section (ts, key, bars) -> playBars bars
        | PlayArpeggio (notes, octave, times, opts) ->
            sprintf "play_pattern_timed [%s],[%s]%s" (scaleToSonicPi notes octave) (generateSonicPiList times)
                (generatePlayOptionsStatments opts)
        | WithFx (fx, opts, sts) ->
            sprintf "with_fx %s%s do\n%send" (fxToSonicPiFx fx) (generateFxOptionsStatments opts)
    ...
    let sonicPiStop =
        ...
        let osc_message =
            OscMessage(sonicPiEndPoint, STOP_COMMAND)

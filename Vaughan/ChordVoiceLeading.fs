...
module ChordVoiceLeading =
    ...
    let private isLeadFunctionOnChordDesiredFunction (chord : Chord)
        desiredNoteFunction desiredPosition =
    ...
    let private inversionForFunction (chord : Chord) desiredNoteFunction
        desiredPosition =
        allInversions chord
        |> List.filter
               (fun (c : Chord) ->
               isLeadFunctionOnChordDesiredFunction c desiredNoteFunction
                   desiredPosition)
        |> List.head

    let private closestChord (minOf : Chord -> Chord -> Chord)
        (list : Chord list) = list |> List.fold minOf (list |> List.head)
    ...
    let private chordWithNoteInDesiredPositionCosestToNote note desiredPosition
        c1 c2 =
        if (distanceBetweenNoteAndChordNote c1 desiredPosition note) < (distanceBetweenNoteAndChordNote
                                                                            c2
                                                                            desiredPosition
                                                                            note) then
            c1
    ...
    let private invertionWithNoteClosestToNote chord note desiredPosition =
        allInversions chord
        |> closestChord
               (chordWithNoteInDesiredPositionCosestToNote note desiredPosition)
    let inversionForFunctionAsLead : InversionForFunctionAsLead =
        fun chord desiredNoteFunction ->
        ...
        fun strategy (chords : Chord List) ->
            ...
            |> List.fold
                   ...
                   if acc.Length = 0 then acc @ [ chord ]
                       ...
                       acc
                       @ [ invertionWithNoteClosestToNote chord
                               (strategy (acc |> List.last)) strategy ]) []

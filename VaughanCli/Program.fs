...
let handleError (e : ArguParseException) (parser : ArgumentParser<_>) subCommand =
    ...
    parser.PrintCommandLineSyntax(usageStringCharacterWidth = 20)
    |> printf "%s\n\n"
    Environment.Exit(-1)

let handleChord (arguments : ParseResults<ChordArguments>)
    (parser : ArgumentParser<CLIArguments>) =
    try
...
let handleTabChord (arguments : ParseResults<ChordArguments>)
    (parser : ArgumentParser<CLIArguments>) =
    try
        let bass =
            arguments.GetResult
                (ChordArguments.Bass, defaultValue = BassStrings.Sixth)
        let shape =
            arguments.GetResult
                (ChordArguments.Shape, defaultValue = ChordShapes.Closed)
        let inversion =
            arguments.GetResult
                (ChordArguments.Inversion, defaultValue = ChordInversions.Root)

        let chord =
            (handleChord arguments parser)
...
let handleTabArpeggio (arguments : ParseResults<ArpeggioArguments>)
    (parser : ArgumentParser<CLIArguments>) =
    try
        let chordArguments = arguments.GetResult ArpeggioArguments.Chord
        let minFret =
            arguments.GetResult(ArpeggioArguments.MinFret, defaultValue = 0)
        let maxFret =
            arguments.GetResult(ArpeggioArguments.MaxFret, defaultValue = 3)
...
let handleTabScale (arguments : ParseResults<ScaleArguments>)
    (parser : ArgumentParser<CLIArguments>) =
    try
        let scaleArguments = arguments.GetResult ScaleArguments.Scale
        let minFret =
            arguments.GetResult(ScaleArguments.MinFret, defaultValue = 0)
        let maxFret =
            arguments.GetResult(ScaleArguments.MaxFret, defaultValue = 3)
...
let handleChords (arguments : ParseResults<ChordsArguments>)
    (parser : ArgumentParser<CLIArguments>) =
    try
        let shape =
            arguments.GetResult
                (ChordsArguments.Shape, defaultValue = ChordShapes.Closed)
        let chordNames =
            arguments.GetResult(ChordsArguments.Chords, defaultValue = [ "" ])
        let toShape = handleChordShape shape
        chordNames |> List.map (parseChord >> toShape)
    with :? ArguParseException as e ->
        handleError e parser Chords
        []

let handleTabChords (arguments : ParseResults<ChordsArguments>)
    (parser : ArgumentParser<CLIArguments>) =
    try
        let bass =
            arguments.GetResult
                (ChordsArguments.Bass, defaultValue = BassStrings.Sixth)
        let toGuitarChord = guitarChord (handleGuitarString bass)
        let chords = handleChords arguments parser
        chords |> List.map (toGuitarChord >> Vaughan.Domain.Chord)
    with :? ArguParseException as e ->
        handleError e parser Chords
        []

let handleTabVoiceLeadChords (arguments : ParseResults<VoiceLeadArguments>)
    (parser : ArgumentParser<CLIArguments>) =
    try
        let bass =
            arguments.GetResult
                (VoiceLeadArguments.Bass, defaultValue = BassStrings.Sixth)
        let leadingVoice =
            arguments.GetResult
                (VoiceLeadArguments.Voice, defaultValue = VoiceLeadOptions.Lead)
...
let handleTabScalesForChord (arguments : ParseResults<ScalesForChordArguments>)
    (parser : ArgumentParser<CLIArguments>) =
    try
        let chordName =
            arguments.GetResult
                (ScalesForChordArguments.Chord, defaultValue = "")
        let minFret =
            arguments.GetResult
                (ScalesForChordArguments.MinFret, defaultValue = 0)
        let maxFret =
            arguments.GetResult
                (ScalesForChordArguments.MaxFret, defaultValue = 3)
...
let handleTabCommonScales (arguments : ParseResults<CommonScalesArguments>)
    (parser : ArgumentParser<CLIArguments>) =
    try
        let chordNames =
            arguments.GetResult
                (CommonScalesArguments.Chords, defaultValue = [ "" ])
        let minFret =
            arguments.GetResult(CommonScalesArguments.MinFret, defaultValue = 0)
        let maxFret =
            arguments.GetResult(CommonScalesArguments.MaxFret, defaultValue = 3)
        ...
        |> commonElements
               (fun scalesPerChord -> scalesPerChord |> List.map scaleName)
        |> Seq.fold (fun r s -> s :: r) []
        |> Seq.toList
        |> List.map (printScaleName
                     >> parseScale
                     >> guitarScale minFret maxFret
                     >> Vaughan.Domain.Scale)
    with :? ArguParseException as e ->
        handleError e parser Chords
        []

let renderTab tab =
    [ StandardTunning; Start ] @ tab @ [ End ]
    |> renderTab
    |> printf "%s\n"

let handleCommand (arguments : ParseResults<CLIArguments>)
    (parser : ArgumentParser<CLIArguments>) renderer =
    ...
    | ScalesForChord sc ->
        (handleTabScalesForChord sc parser)
        |> List.iter (fun t ->
               printf "%s\n" (fst t)
               renderer [ snd t ])
    0
...
let main argv =
    let parser =
        ArgumentParser.Create<CLIArguments>
            (programName = "VaughanCLI", helpTextMessage = "Guitar tab CLI")
    try

...
module ChordTests =
    ...
    [<Property>]
    let ``Chords should be named after the root`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
        (name chord) |> should startWith (noteName root)

    [<Property>]
    let ``Major chords should be named after the quality`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
        (quality = Major || quality = Major7 || quality = Major9
         || quality = Major9Sharp11 || quality = Major11
         || quality = Major13Sharp11)
        ==> lazy ((name chord).StartsWith((noteName root) + "Maj"))

    [<Property>]
    let ``Minor chords should be named after the quality`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
        (quality = Minor || quality = Minor6 || quality = Minor6Add9
         || quality = Minor7 || quality = Minor7b5 || quality = Minor9
         || quality = MinorMaj7 || quality = MinorMaj9)
        ==> lazy ((name chord).StartsWith((noteName root) + "Min"))

    [<Property>]
    let ``Dominant chords should be named after the quality`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
        (quality = Dominant7 || quality = Dominant9 || quality = Dominant11
         || quality = Dominant13 || quality = Dominant7Flat5
         || quality = Dominant7Flat5Flat9 || quality = Dominant7Flat5Sharp9
         || quality = Dominant7Flat9 || quality = Dominant7Sharp9)
        ==> lazy
            ((name chord).StartsWith((noteName root) + "7")
    ...
    let ``Should filter function from chord``() =
        (c9 |> skipFunction Fifth).Notes
        |> should equal [ (C, Root)
                          (E, Third)
                          (BFlat, Seventh)
                          (D, Ninth) ]
        (c9 |> skipFunction Root).Notes
        |> should equal [ (E, Third)
                          (G, Fifth)
                          (BFlat, Seventh)
                          (D, Ninth) ]
        (c9 |> skipFunction Seventh).Notes
        |> should equal [ (C, Root)
                          (E, Third)
                          (G, Fifth)
                          (D, Ninth) ]
    ...
    [<Property>]
    let ``Should put third on bass for first inversion`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    let ``Should invert chord for second inversion``() =
        (cMaj
         |> invert
         |> invert).Notes
        |> should equal [ (G, Fifth)
                          (C, Root)
                          (E, Third) ]
        (cAug
         |> invert
         |> invert).Notes
        |> should equal [ (GSharp, Fifth)
                          (C, Root)
                          (E, Third) ]
        (cMin
         |> invert
    ...
    [<Property>]
    let ``Should put fifth on bass for second inversion`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
        snd (chord
             |> invert
             |> invert).Notes.Head = Fifth
    ...
    [<Property>]
    let ``Should put sixth or seventh on bass for third inversion`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
        ...
        ==> lazy
            (snd (chord
                  |> invert
                  |> invert
                  |> invert).Notes.Head = Seventh
             || snd (chord
                     |> invert
                     |> invert
                     |> invert).Notes.Head = Sixth)
    ...
    [<Property>]
    let ``Drop 2 chords have third on lead and root on bass`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``Drop 3 chords have fifth on lead`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``First inversion drop 2 chords have fifth on lead and third on bass`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``Second inversion drop 2 chords have seventh or sixth on lead and fifth on bass`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
        ...
        ==> lazy (bass = Fifth && (lead = Seventh || lead = Sixth))

    [<Property>]
    let ``Third inversion drop 2 chords have root on lead and seventh or sixth on bass`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
        ...
        let bass = snd (drop2Chord.Notes.Head)
        (chord.Notes.Length = 4)
        ==> lazy ((bass = Seventh || bass = Sixth) && lead = Root)

    [<Property>]
    let ``Fourth inversion drop 2 chords should have notes in same position as univerted chord`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``First inversion drop 3 chords have third on lead and seventh or sixth on bass`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
        ...
        let bass = snd (drop3Chord.Notes.Head)
        (chord.Notes.Length = 4)
        ==> lazy (bass = Third && (lead = Seventh || lead = Sixth))

    [<Property>]
    let ``Second inversion drop 3 chords have seventh or sixth on lead and third on bass`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``Third inversion drop 3 chords have third on lead and seventh or sixth on bass`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
        ...
        let bass = snd (drop3Chord.Notes.Head)
        (chord.Notes.Length = 4)
        ==> lazy ((bass = Seventh || bass = Sixth) && lead = Third)

    [<Property>]
    let ``Fourth inversion drop 3 chords should have notes in same position as univerted chord`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    let ``Should choose invertion that satisfies having a specific function as lead``() =
        (inversionForFunctionAsLead cMaj Third).Notes
        |> should equal (cMaj
                         |> invert
                         |> invert).Notes

    [<Property>]
    let ``Should choose invertion that satisfies having a root as lead`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``Should choose invertion that satisfies having a third as lead`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``Should choose invertion that satisfies having a fifth as lead`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    let ``Should choose invertion that satisfies having a specific function as bass``() =
        (inversionForFunctionAsBass cMaj Fifth).Notes
        |> should equal (cMaj
                         |> invert
                         |> invert).Notes

    [<Property>]
    let ``Should choose invertion that satisfies having a root as bass`` (root : Note)
        (quality : ChordQuality) =
    ...
    [<Property>]
    let ``Should choose invertion that satisfies having a third as bass`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``Should choose invertion that satisfies having a fifth as bass`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``It should choose invertion that satisfies having a lead that is closest to a provided note`` (root : Note)
        (quality : ChordQuality) (providedNote : Note) =
        let chord = chord root quality
        ...
        (distancesToProvidedNote |> List.min) = (distancesToProvidedNote
                                                 |> List.last)

    [<Property>]
    let ``It should choose invertion that satisfies having a bass that is closest to a provided note`` (root : Note)
        (quality : ChordQuality) (providedNote : Note) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``It should return an empty list of fitting chords for list of notes with two notes`` (note1 : Note)
        (note2 : Note) =
        let fittingChords = chordsFitting [ note1; note2 ]
        fittingChords |> should be Empty

    [<Property>]
    let ``It should return the fitting chord for list of notes from triad`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``It should return the chord for list of notes from first inversion triad`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``It should return the chord for list of notes from second inversion triad`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``It should return the chord for list of notes from seventh chord`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
        let chordNotes = chord.Notes |> List.map fst
    ...
    [<Property>]
    let ``It should return the chord for list of notes from seventh chord in first inversion`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
        let invertedChord = chord |> invert
        let chordNotes = invertedChord.Notes |> List.map fst
    ...
    [<Property>]
    let ``It should return the chord for list of notes from seventh chord in second inversion`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality
    ...
    [<Property>]
    let ``It should return the chord for list of notes from seventh chord in third inversion`` (root : Note)
        (quality : ChordQuality) =
        let chord = chord root quality

...
module NoteTests =
    ...
    let ``Should create beats from timeSignature and duration``() =
        ...
        (durationToBeats (4.0<beat>, Quarter) Sixteenth)
        |> should equal 0.25<beat>
        (durationToBeats (4.0<beat>, Quarter) ThirtySecond)
        |> should equal 0.125<beat>
    ...
    let ``Two semi tones difference between two notes is a major second interval`` (note : Note) =
        (intervalBetween note (note
                               |> sharp
                               |> sharp)) = MajorSecond
        && (intervalBetween (note
                             |> flat
                             |> flat) note) = MajorSecond
    ...
    let ``Should have frequency of 55Hz for A1``() =
        (float (440.0<hz> / 8.0))
        |> should (equalWithin 0.1) (float (frequency A Octave.Contra))

    [<Fact>]
    let ``Should have frequency of 110Hz for A2``() =
        (float (440.0<hz> / 4.0))
        |> should (equalWithin 0.1) (float (frequency A Octave.Great))

    [<Fact>]
    let ``Should have frequency of 220Hz for A3``() =
        (float (440.0<hz> / 2.0))
        |> should (equalWithin 0.1) (float (frequency A Octave.Small))

    [<Fact>]
    let ``Should have frequency of 440Hz for A4``() =
        (float (440.0<hz> / 1.0))
        |> should (equalWithin 0.1) (float (frequency A Octave.OneLine))

    [<Fact>]
    let ``Should have frequency of 880Hz for A5``() =
        (440.0 * 2.0)
        |> should (equalWithin 0.1) (float (frequency A Octave.TwoLine))

    [<Fact>]
    let ``Should have frequency of 1760Hz for A6``() =
        (440.0 * 4.0)
        |> should (equalWithin 0.1) (float (frequency A Octave.ThreeLine))

    [<Fact>]
    let ``Should have frequency of 7040Hz for A8``() =
        (440.0 * 16.0)
        |> should (equalWithin 0.1) (float (frequency A Octave.FiveLine))

    [<Fact>]
    let ``Should have frequency of 14080Hz for A9``() =
        (440.0 * 32.0)
        |> should (equalWithin 0.1) (float (frequency A Octave.SixLine))

    [<Fact>]
    let ``Should have frequency of 3520Hz for A7``() =
        (440.0 * 8.0)
        |> should (equalWithin 0.1) (float (frequency A Octave.FourLine))

    [<Fact>]
    let ``Should have frequency of 261.626Hz for C4``() =
        (261.626)
        |> should (equalWithin 0.1) (float (frequency C Octave.OneLine))

    [<Property>]
    let ``Small octave divides by 2 frequency of One line octave`` (note : Note) =
        (float (frequency note Octave.OneLine / 2.0))
        |> should (equalWithin 0.1) (float (frequency note Octave.Small))

    [<Property>]
    let ``Great octave divides by 4 frequency of One line octave`` (note : Note) =
        (float (frequency note Octave.OneLine / 4.0))
        |> should (equalWithin 0.1) (float (frequency note Octave.Great))

    [<Property>]
    let ``Contra octave divides by 8 frequency of One line octave`` (note : Note) =
        (float (frequency note Octave.OneLine / 8.0))
        |> should (equalWithin 0.1) (float (frequency note Octave.Contra))

    [<Property>]
    let ``Two line octave doubles frequency of One line octave`` (note : Note) =
        (float (frequency note Octave.OneLine * 2.0))
        |> should (equalWithin 0.1) (float (frequency note Octave.TwoLine))

    [<Property>]
    let ``Three line octave quadruples frequency of One line octave`` (note : Note) =
        (float (frequency note Octave.OneLine * 4.0))
        |> should (equalWithin 0.1) (float (frequency note Octave.ThreeLine))

    [<Property>]
    let ``Four line octave multiplies by 8 frequency of One line octave`` (note : Note) =
        (float (frequency note Octave.OneLine * 8.0))
        |> should (equalWithin 0.1) (float (frequency note Octave.FourLine))

    [<Property>]
    let ``Five line octave multiplies by 16 frequency of One line octave`` (note : Note) =
        (float (frequency note Octave.OneLine * 16.0))
        |> should (equalWithin 0.1) (float (frequency note Octave.FiveLine))

    [<Property>]
    let ``Six line octave multiplies by 32 frequency of One line octave`` (note : Note) =
        (float (frequency note Octave.OneLine * 32.0))
        |> should (equalWithin 0.1) (float (frequency note Octave.SixLine))
    ...
    let ``Should have midi number from 0 to 11 on sub contra octave`` (note : Note) =
        (midiNumber note Octave.SubContra)
        |> should be (greaterThan -1<midiNote>)
    ...
    let ``Should have midi number from 72 to 83 on three line octave`` (note : Note) =
        (midiNumber note Octave.ThreeLine)
        |> should be (greaterThan 71<midiNote>)
    ...
    let ``Should have midi number from 84 to 95 on four line octave`` (note : Note) =
        (midiNumber note Octave.FourLine)
        |> should be (greaterThan 83<midiNote>)
    ...
    let ``Should have midi number from 96 to 107 on five line octave`` (note : Note) =
        (midiNumber note Octave.FiveLine)
        |> should be (greaterThan 95<midiNote>)
    ...
    let ``Should have midi number from 108 to 119 on six line octave`` (note : Note) =
        (midiNumber note Octave.SixLine)
        |> should be (greaterThan 107<midiNote>)
    ...
    let ``Should have midi number from 120 to 131 on seven line octave`` (note : Note) =
        (midiNumber note Octave.SevenLine)
        |> should be (greaterThan 119<midiNote>)

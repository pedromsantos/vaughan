...
module GuitarTests =
    ...
    let ``Should map diatonic closed triads to guitar fretboard`` (scaleType : ScaleType)
        (scaleDegree : ScaleDegree) (root : Note) (bassString : GuitarString) () =
        ((bassString <> FirstString || bassString = SecondString)) ==> lazy
    ...
    let ``Should map c major to guitar fretboard``() =
        (guitarChord SixthString cMaj).Frets
        |> should equal [ { GuitarString = SixthString
                            Fret = 8
                            Note = C }
    ...
    let ``Should map c major open to guitar fretboard on fifth string``() =
        (guitarChord FifthString (cMaj |> toOpen)).Frets
        |> should equal [ { GuitarString = FifthString
                            Fret = 3
                            Note = C }
                          { GuitarString = FourthString
                            Fret = 2
    ...
    let ``Should map c major to guitar fretboard on fourth string``() =
        (guitarChord FourthString cMaj).Frets
        |> should equal [ { GuitarString = FourthString
                            Fret = 10
                            Note = C }
                          { GuitarString = ThirdString
                            Fret = 9
    ...
    let ``Should map c major to guitar fretboard on third string``() =
        (guitarChord ThirdString cMaj).Frets
        |> should equal [ { GuitarString = ThirdString
                            Fret = 5
                            Note = C }
                          { GuitarString = SecondString
                            Fret = 5
    ...
    let ``Should map F major to guitar fretboard on sixth string``() =
        ...
        (guitarChord SixthString fMaj).Frets
        |> should equal [ { GuitarString = SixthString
                            Fret = 13
                            Note = F }
                          { GuitarString = FifthString
    ...
    [<Property(Arbitrary = [| typeof<DiatonicScales> |])>]
    let ``Should map diatonic closed sevent chords to guitar fretboard`` (scaleType : ScaleType)
        (scaleDegree : ScaleDegree) (root : Note) (bassString : GuitarString) () =
        ((bassString = SixthString || bassString = FifthString
          || bassString = FourthString)) ==> lazy
            (let scale = createScaleNotes scaleType root
    ...
    [<Property(Arbitrary = [| typeof<DiatonicScales> |])>]
    let ``Should map diatonic closed seventh drop 2 chords to guitar fretboard`` (scaleType : ScaleType)
        (scaleDegree : ScaleDegree) (root : Note) (bassString : GuitarString) () =
        ((bassString = SixthString || bassString = FifthString
          || bassString = FourthString)) ==> lazy
            (let scale = createScaleNotes scaleType root
    ...
    let ``Should map C major 7 drop 2 to guitar fretboard on fifth string closed``() =
        ...
        (guitarChord FifthString chord).Frets
        |> should equal [ { GuitarString = FifthString
                            Fret = 3
                            Note = C }
                          { GuitarString = FourthString
    ...
    [<Property(Arbitrary = [| typeof<DiatonicScales> |])>]
    let ``Should map diatonic closed sevent drop 3 chords to guitar fretboard`` (scaleType : ScaleType)
        (scaleDegree : ScaleDegree) (root : Note) (bassString : GuitarString) () =
        (bassString = SixthString || bassString = FifthString) ==> lazy
            (let scale = createScaleNotes scaleType root
    ...
    let ``Should map C major 7 drop 3 to guitar fretboard on sixth string closed``() =
        ...
        (guitarChord SixthString chord).Frets
        |> should equal [ { GuitarString = SixthString
                            Fret = 8
                            Note = C }
                          { GuitarString = FifthString
    ...
    let ``Should map C major9 ignoring 5th to guitar fretboard on fifth string closed``() =
        let chord = chord C Major9 |> skipFunction Fifth
        (guitarChord FifthString chord).Frets
        |> should equal [ { GuitarString = FifthString
                            Fret = 3
                            Note = C }
    ...
    let ``Should map C9 ignoring 5th to guitar fretboard on fifth string closed``() =
        let chord = chord C Dominant9 |> skipFunction Fifth
        (guitarChord FifthString chord).Frets
        |> should equal [ { GuitarString = FifthString
                            Fret = 3
                            Note = C }
    ...
    [<Property(Arbitrary = [| typeof<DiatonicScales> |])>]
    let ``Should map diatonic closed triad arpeggios to guitar fretboard first position`` (scaleType : ScaleType)
        (scaleDegree : ScaleDegree) (root : Note) () =
        let scale = createScaleNotes scaleType root
        let chord = triadsHarmonizer scaleDegree scale
        let guitarArpeggio = guitarArpeggio 1 4 chord
        let unmutedFrets =
            guitarArpeggio.ArpeggioFrets |> List.filter (fun f -> f.Fret > -1)
...
module GuitarTabTests =
    ...
    let createTriad (scaleType : ScaleType) (scaleDegree : ScaleDegree)
        (root : Note) (bassString : GuitarString) =
        let scale = createScaleNotes scaleType root
        let chord = triadsHarmonizer scaleDegree scale
        guitarChord bassString chord

    let createSeventhChord (scaleType : ScaleType) (scaleDegree : ScaleDegree)
        (root : Note) (bassString : GuitarString) =
        let scale = createScaleNotes scaleType root
        let chord = seventhsHarmonizer scaleDegree scale
        guitarChord bassString chord
    ...
    [<Property(Arbitrary = [| typeof<DiatonicScales> |])>]
    let ``Should map diatonic closed triad to guitar tab`` (scaleType : ScaleType)
        (scaleDegree : ScaleDegree) (root : Note) (bassString : GuitarString) () =
    ...
    [<Property(Arbitrary = [| typeof<DiatonicScales> |])>]
    let ``Should map diatonic closed seventh chord to guitar tab`` (scaleType : ScaleType)
        (scaleDegree : ScaleDegree) (root : Note) (bassString : GuitarString) () =
        (bassString <> ThirdString && bassString <> SecondString
         && bassString <> FirstString)
        ==> lazy
            (let guitarChord =
                 createSeventhChord scaleType scaleDegree root bassString
             let frets = guitarChord.Frets
             let tab = tabifyChord guitarChord
             tab.Contains(string frets.[0].Fret)
             && tab.Contains(string frets.[1].Fret)
    ...
    let ``Should draw C major 7 drop 2 to guitar fretboard on fifth string closed ``() =
        ...
        |> tabifyChord
        |> should equal
               ("e||-----||" + Environment.NewLine + "B||--5--||"
                + Environment.NewLine + "G||--4--||" + Environment.NewLine
                + "D||--5--||" + Environment.NewLine + "A||--3--||"
    ...
    let ``Should draw A major 7 to guitar fretboard on fifth string closed ``() =
        ...
        |> tabifyChord
        |> should equal
               ("e||------||" + Environment.NewLine + "B||--14--||"
                + Environment.NewLine + "G||--13--||" + Environment.NewLine
                + "D||--14--||" + Environment.NewLine + "A||--12--||"
                + Environment.NewLine + "E||------||" + Environment.NewLine)
    ...
    let ``Should draw c major to guitar fretboard on sixth string``() =
        ...
        |> should equal
               ("e||-----||" + Environment.NewLine + "B||-----||"
                ...
                + "D||--5--||" + Environment.NewLine + "A||--7--||"
                + Environment.NewLine + "E||--8--||" + Environment.NewLine)
    ...
    let ``Should draw C major 7 drop 3 to guitar fretboard on sixth string closed``() =
        ...
        |> tabifyChord
        |> should equal
               ("e||-----||" + Environment.NewLine + "B||--8--||"
                + Environment.NewLine + "G||--9--||" + Environment.NewLine
                + "D||--9--||" + Environment.NewLine + "A||-----||"
                + Environment.NewLine + "E||--8--||" + Environment.NewLine)
    ...
    let ``Should draw C major 7 drop 3 to guitar fretboard on fifth string closed``() =
        ...
        |> tabifyChord
        |> should equal
               ("e||--3--||" + Environment.NewLine + "B||--5--||"
                + Environment.NewLine + "G||--4--||" + Environment.NewLine
                + "D||-----||" + Environment.NewLine + "A||--3--||"
                + Environment.NewLine + "E||-----||" + Environment.NewLine)
    ...
    let ``Should map C9 ignoring 5th to guitar fretboard on fifth string closed``() =
        ...
        |> tabifyChord
        |> should equal
               ("e||-----||" + Environment.NewLine + "B||--3--||"
                + Environment.NewLine + "G||--3--||" + Environment.NewLine
                + "D||--2--||" + Environment.NewLine + "A||--3--||"
                + Environment.NewLine + "E||-----||" + Environment.NewLine)
    ...
    let ``Should tabify multiple chords``() =
        ...
        let guitarChords =
            ...
            |> List.map (toDrop2 >> (guitarChord FifthString))
    ...
    let ``Should draw shape of C major 7 drop 3 on sixth string``() =
        ...
        |> shapify
        |> should equal
               ("CMaj7" + Environment.NewLine + "EADGBE" + Environment.NewLine
    ...
    let ``Should render standart tuning as tab``() =
        ...
        |> renderTab
        |> should equal
               ("e-||" + Environment.NewLine + "B-||" + Environment.NewLine
                + "G-||" + Environment.NewLine + "D-||" + Environment.NewLine
                + "A-||" + Environment.NewLine + "E-||" + Environment.NewLine)
    ...
    let ``Should render muted c note to guitar fretboard on sixth string``() =
        ...
        |> should equal
               ("e||-----||" + Environment.NewLine + "B||-----||"
                + Environment.NewLine + "G||-----||" + Environment.NewLine
                + "D||-----||" + Environment.NewLine + "A||-----||"
                + Environment.NewLine + "E||--x8--||" + Environment.NewLine)

    [<Fact>]
    let ``Should render palm muted c note to guitar fretboard on sixth string``() =
        let guitarNote =
            PalmMute({ GuitarString = SixthString
                       Fret = 8
                       Note = C })
        [ StandardTunning; Start; guitarNote; End ]
        |> renderTab
        |> should equal
               ("e||-----||" + Environment.NewLine + "B||-----||"
                + Environment.NewLine + "G||-----||" + Environment.NewLine
                + "D||-----||" + Environment.NewLine + "A||-----||"
                + Environment.NewLine + "E||--_8--||" + Environment.NewLine)
    ...
    let ``Should render hammer on from c note to d note guitar fretboard on sixth string``() =
        let guitarNote =
            HammerOn(C |> guitarNote SixthString, D |> guitarNote SixthString)
        [ StandardTunning; Start; guitarNote; End ]
        |> renderTab
        |> should equal
               ("e||---------||" + Environment.NewLine + "B||---------||"
                + Environment.NewLine + "G||---------||" + Environment.NewLine
                + "D||---------||" + Environment.NewLine + "A||---------||"
                + Environment.NewLine + "E||--8-h-10--||" + Environment.NewLine)

    [<Fact>]
    let ``Should render pull of from d note to c note guitar fretboard on sixth string``() =
        let guitarNote =
            PullOff(D |> guitarNote SixthString, C |> guitarNote SixthString)
        [ StandardTunning; Start; guitarNote; End ]
        |> renderTab
        |> should equal
               ("e||---------||" + Environment.NewLine + "B||---------||"
                + Environment.NewLine + "G||---------||" + Environment.NewLine
                + "D||---------||" + Environment.NewLine + "A||---------||"
                + Environment.NewLine + "E||--10-p-8--||" + Environment.NewLine)

    [<Fact>]
    let ``Should rende c note bended to d guitar fretboard on sixth string``() =
        let guitarNote =
            Bend(C |> guitarNote SixthString, D |> guitarNote SixthString)
        [ StandardTunning; Start; guitarNote; End ]
        |> renderTab
        |> should equal
               ("e||---------||" + Environment.NewLine + "B||---------||"
                + Environment.NewLine + "G||---------||" + Environment.NewLine
                + "D||---------||" + Environment.NewLine + "A||---------||"
                + Environment.NewLine + "E||--8-b-10--||" + Environment.NewLine)
    ...
    let ``Should render C major arpeggio to guitar fretboard on open position ``() =
        [ StandardTunning
          Start
          Arpeggio(guitarArpeggio 0 3 cMaj)
          End ]
        |> renderTab
        |> should equal

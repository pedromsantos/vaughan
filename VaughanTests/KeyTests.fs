...
module KeyTests =
    ...
    let ``Should have notes for key``() =
        ...
        keyNotes EMajor
        |> should equal [ E; FSharp; GSharp; A; B; CSharp; DSharp ]
        keyNotes BMajor
        |> should equal [ B; CSharp; DSharp; E; FSharp; GSharp; ASharp ]
        keyNotes FSharpMajor
        |> should equal [ FSharp; GSharp; ASharp; B; CSharp; DSharp; F ]
        keyNotes DFlatMajor
        |> should equal [ DFlat; EFlat; F; GFlat; AFlat; BFlat; C ]
        keyNotes AFlatMajor
        |> should equal [ AFlat; BFlat; C; DFlat; EFlat; F; G ]
        keyNotes GFlatMajor
        |> should equal [ GFlat; AFlat; BFlat; B; DFlat; EFlat; F ]
        ...
        keyNotes FSharpMinor
        |> should equal [ FSharp; GSharp; A; B; CSharp; D; E ]
        keyNotes CSharpMinor
        |> should equal [ CSharp; DSharp; E; FSharp; GSharp; A; B ]
        keyNotes GSharpMinor
        |> should equal [ GSharp; ASharp; B; CSharp; DSharp; E; FSharp ]
        keyNotes EFlatMinor
        |> should equal [ EFlat; F; GFlat; AFlat; BFlat; B; DFlat ]
        keyNotes BFlatMinor
        |> should equal [ BFlat; C; DFlat; EFlat; F; GFlat; AFlat ]
    ...
    let ``Major keys have formula R, W, W, H, W, W, W, H and Minor keys have formula R, W, H, W, W, H, W, W`` (key : Key) =
        let majorKeyformula =
            [ Unisson; MajorSecond; MajorSecond; MinorSecond; MajorSecond;
              MajorSecond; MajorSecond; MinorSecond ]
        let minorKeyformula =
            [ Unisson; MajorSecond; MinorSecond; MajorSecond; MajorSecond;
        ...
        let intervalsForKey =
            ...
            |> List.mapi (fun i n ->
                   if i = 0 then Unisson
                   else intervalBetween notesForKeyWithOctave.[i - 1] n)

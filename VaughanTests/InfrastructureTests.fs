...
[<Fact>]
let ``Should swap second two elements in list`` () = swapSecondTwo [ 1; 2; 3 ] = [ 1; 3; 2 ]
...
let ``Should filter odd index elements`` () =
    filterOddIndexElements { 1 .. 6 }
    |> List.ofSeq = ({ 1 .. 2 .. 6 } |> List.ofSeq)

[<Fact>]
let ``Should create circular sequence from list`` () =
    circularSequenceFromList [ 1; 2; 3 ]
    |> Seq.take 3
    |> Seq.last
    |> should equal 3
    circularSequenceFromList [ 1; 2; 3 ]

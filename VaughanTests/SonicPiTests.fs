...
module SonicPiTests =
    ...
    let ``Should generate SonicPi DSL for play note with amplitude 2``() =
        PlayNote(C, OneLine, [ Amplitude(2.0<loud>) ])
        |> toSonicPiScript
        |> should equal "play 48,amp:2.00"

    [<Fact>]
    let ``Should generate SonicPi DSL for play note with panning rigth``() =
        PlayNote(C, OneLine, [ Panning(1.0<pan>) ])
        |> toSonicPiScript
        |> should equal "play 48,pan:1.00"

    [<Fact>]
    let ``Should generate SonicPi DSL for play note with release 2``() =
        PlayNote(C, OneLine, [ Release(2.0<beat>) ])
        |> toSonicPiScript
        |> should equal "play 48,release:2.00"

    [<Fact>]
    let ``Should generate SonicPi DSL for play note with attack 2``() =
        PlayNote(C, OneLine, [ Attack(2.0<beat>) ])
        |> toSonicPiScript
        |> should equal "play 48,attack:2.00"

    [<Fact>]
    let ``Should generate SonicPi DSL for play note with attack level 2``() =
        PlayNote(C, OneLine, [ AttackLevel(2.0<beat>) ])
        |> toSonicPiScript
        |> should equal "play 48,attack_level:2.00"

    [<Fact>]
    let ``Should generate SonicPi DSL for play note with sustain 2``() =
        PlayNote(C, OneLine, [ Sustain(2.0<beat>) ])
        |> toSonicPiScript
        |> should equal "play 48,sustain:2.00"

    [<Fact>]
    let ``Should generate SonicPi DSL for play note with sustain level 2``() =
        PlayNote(C, OneLine, [ SustainLevel(2.0<beat>) ])
        |> toSonicPiScript
        |> should equal "play 48,sustain_level:2.00"

    [<Fact>]
    let ``Should generate SonicPi DSL for play note with decay 2``() =
        PlayNote(C, OneLine, [ Decay(2.0<beat>) ])
        |> toSonicPiScript
        |> should equal "play 48,decay:2.00"

    [<Fact>]
    let ``Should generate SonicPi DSL for play note with decay level 2``() =
        PlayNote(C, OneLine, [ DecayLevel(2.0<beat>) ])
        |> toSonicPiScript
        |> should equal "play 48,decay_level:2.00"
    ...
    let ``Should generate SonicPi DSL for play sample``() =
        ...
        |> toSonicPiScript
        |> should equal "sample :loop_garzul"
